class Student():
    ...
    def calcGPA(self):
        '''Calculate the students gpa SM and DH'''
        totalPoints = 0.0
        totalCreds = 0
        for course, grade in self.courses.items():
            gp = self._grade_points.get(grade, 0.0)
            cred = course.cred if hasattr(course, 'cred') else 0
            totalPoints += gp * cred
            totalCreds += cred
        return round(totalPoints / totalCreds, 2) if totalCreds > 0 else 0.0
